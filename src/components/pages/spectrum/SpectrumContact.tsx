...
const SpectrumContact: React.FC = () => {
  ...
  const [phone, setPhone] = useState("72006 55336");
  const [email, setEmail] = useState("pyramidspectrum@gmail.com");
  ...
  const [branchLocations, setBranchLocations] = useState(locations);
      ...
      .then(data => {
        if (data && !data.error) {
          ...
          if (data.addresses && Array.isArray(data.addresses)) {
            setBranchLocations(data.addresses);
          }
        }
      })
      .catch(err => console.error("Error fetching contact info from MongoDB:", err));
  }, []);

  const handleShowMap = (locationId: string) => {
    const location = branchLocations.find(loc => loc.id === locationId);
    if (location) {
    ...
    <div className="contact-page">
      ...
      <div className="container">
        ...
        <div className="page-header text-center">
          <h1>Contact Us</h1>
          <p>Get in touch with <b style={{ color: '#2a50a2' }}>Our Spectrum Team</b> for admissions, queries, and support</p>
        </div>
              ...
              <div className="contact-info">
                ...
                <div className="contact-methods">
                  {branchLocations.map((loc) => (
                    <div className="contact-item" key={loc.id}>
                ...
                <div className="social-media">
                  ...
                  <div className="social-icons">
                    <a href={socialLinks.facebook} aria-label="Facebook" className="social-icon" target="_blank" rel="noreferrer">
                      <i className="fab fa-facebook-f"></i>
                    </a>
                    <a href={socialLinks.instagram} aria-label="Instagram" className="social-icon" target="_blank" rel="noreferrer">
                      <i className="fab fa-instagram"></i>
                    </a>
                    <a href={socialLinks.youtube} aria-label="YouTube" className="social-icon" target="_blank" rel="noreferrer">
                      <i className="fab fa-youtube"></i>
                    </a>
                    <a href={socialLinks.linkedin} aria-label="LinkedIn" className="social-icon" target="_blank" rel="noreferrer">
                ...
                <div className="location-cards-container mt-4">
                  ...
                  {branchLocations.map((loc) => (
                    <div className="center-card mb-3" key={loc.id}>
                      <div className="text-center mb-3">
                        <i className="fas fa-map-marker-alt fa-2x"></i>
                      </div>
                      <h3 className="text-center">{loc.name}</h3>
                      <p className="address text-center">
                        {loc.address.split('. ').map((part, i) => (
                          <React.Fragment key={i}>
                            {part}
                            {i < loc.address.split('. ').length - 1 && <br />}
                          </React.Fragment>
                        ))}
                      </p>
                      <p className="phone text-center">
                        <i className="fas fa-phone-alt"></i> {loc.phone}
                      </p>
