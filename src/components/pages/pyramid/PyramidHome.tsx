...
// Pyramid-specific hero banner images from /public/pyramid-banners/
const pyramidHeroBanners = [
  { src: '/pyramid-banners/banner-4.jpg',         alt: 'Pyramid Banner' },
  { src: '/pyramid-banners/banner-1.jpg',         alt: 'Pyramid Banner' },
  { src: '/pyramid-banners/banner-3.jpg',         alt: 'Pyramid Banner' },
  { src: '/pyramid-banners/academics.png',        alt: 'Academics' },
  { src: '/pyramid-banners/foundation.jpg',       alt: 'Foundation Program' },
  { src: '/pyramid-banners/basic-english.jpg',    alt: 'Basic English' },
  { src: '/pyramid-banners/advanced-english.jpg', alt: 'Advanced English' },
  { src: '/pyramid-banners/practical-english.jpg',alt: 'Practical English' },
  { src: '/pyramid-banners/power.webp',           alt: 'Power Program' },
];
...
const PyramidHome: React.FC = () => {
  ...
  // Dynamic MongoDB content state — start null so only API data is shown
  const [heroTitle, setHeroTitle] = useState<string | null>(null);
  const [heroSubtitle, setHeroSubtitle] = useState<string | null>(null);
  const [banners, setBanners] = useState<Array<{ src: string; alt: string }>>([]);
  const [homeLoading, setHomeLoading] = useState(true);

  // Fetch from MongoDB via Flask API — API response is the primary source of truth
  useEffect(() => {
    fetch(`${API_URL}/home/pyramid`)
      .then(res => res.json())
      .then(data => {
        if (data && !data.error) {
          if (data.hero_section) {
            setHeroTitle(data.hero_section.title || null);
            setHeroSubtitle(data.hero_section.subtitle || null);
          }
          if (data.banner_images && Array.isArray(data.banner_images) && data.banner_images.length > 0) {
            setBanners(data.banner_images);
          } else {
            // Fallback to local banners only when API returns no banners
            setBanners(pyramidHeroBanners);
          }
        } else {
          // API error — fall back to local banners so page isn't empty
          setBanners(pyramidHeroBanners);
        }
        setHomeLoading(false);
      })
      .catch(err => {
        console.error("Error fetching home content from MongoDB:", err);
        setBanners(pyramidHeroBanners);
        setHomeLoading(false);
      });
  }, []);
  ...
  // FAQ data (kept for future FAQ section)
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const faqItems = [
    {
  ...
  const handleScholarshipTabClick = (tab: string) => {
    ...
    navigate('/pyramid/course-registration');
  };
        ...
        <section className="scholarship-hero">
          <div className="container">
            <div className="row align-items-center">
              ...
              {homeLoading
                ? <span className="hero-loading-pulse">Loading...</span>
                : <span className="highlight-spectrum fw-bolder">{heroTitle}</span>
              }
              </h1>
              <div className="scholarship-tabs-container">
                <p className="hero-subtitle text-center">
                  {homeLoading
                    ? <span className="hero-loading-pulse">Fetching content...</span>
                    : heroSubtitle
                  }
                </p>
                ...
                <div className="scholarship-carousel-indicators">
                  <ul>
                    {banners.map((_, idx) => (
                      <li
                        key={idx}
                        className={scholarshipIndex === idx ? 'active' : ''}
                        onClick={() => setScholarshipIndex(idx)}
                      ></li>
                    ))}
                  </ul>
                </div>
        ...
        <section className="section admissions-section">
          <div className="container">
            ...
            <div className="text-center mt-4">
              <AnimatedButton 
                text="Register Now" 
                href="/pyramid/course-registration"
                className="explore-courses-animated-btn"
              />
            </div>
          </div>
